...
class Adapter {
	constructor(adapterName, adapter, input) {
		//normalConsole({adapterName, adapter, input})
		...
		if (this.config) {
			...
			if (template.status === 'failed') {
				...
				try {
					...
					normalConsole({ ADAPTER_CONVERSION_ERROR: e })
					return {
						status: "failed",
						message: "couldnt load profile"
					}
			...
			for (let rule of rules) {
				if (rule.name === item) {
					...
					if (result.value == 'undefined') {
						result.value = this.dot.pick(rule.path.trim(), this.input.customerData)
						// normalConsole('################################### Templatexx');
						// normalConsole(JSON.stringify(result.value)); 
						// normalConsole('################################### Template Endxx')
					} else {
						// normalConsole('################################### Template');
						// normalConsole(JSON.stringify(result.value)); 
						// normalConsole('################################### Template End')
					}
					if (result.value !== 'undefined') {
						matched = true
						...
						//value parsers
						if (rule['item-delimiter'] && result.value) {
							...
							//the majestic trickiness begins here!!!						
							...
							if (parsedArray.length > 0) {
								...
								for (let index in parsedArray) {
									...
									let metaData = rule['option-meta-data'] || false

									if (metaData) {
										//TODO: Document this -> mutation was occurring here when we didnt use object.assign
										let meta = Object.assign({}, metaData)
										let replacementName = meta.value
										let replacementValue = obj[replacementName]
										meta.value = replacementValue || 'KES'
										obj['meta'] = [meta]
									}
	...
	merge(data) {
		...
		// 		normalConsole('=================================================');
		// 		normalConsole(JSON.stringify(data)+ '@@@@@merge data'); 
		//    normalConsole('=================================================');
